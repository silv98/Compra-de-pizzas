...
//Função de mapeamento do item no JSon
pizzaJson.map((item, index) => {
    
// preencher as informações em pizzaItem
  let pizzaItem = c('.models .pizza-item').cloneNode(true);
 
  //Setando a chave da pizza para saber qual item eu cliquei
  pizzaItem.setAttribute('data-key', index);

  //Informação das pizzas no modal
      //selecionando imagem na tela para substituição das imagens da pizza. Neste campo tem de buscar a TAG img
  pizzaItem.querySelector('.pizza-item--img img').src = item.img;
  
      //selecionando o preço do item. Usando também o template String
  pizzaItem.querySelector('.pizza-item--price').innerHTML = `R$ ${item.price.toFixed(2)}`;
  
  //selecionando o nome do item. Usando também o template String
  pizzaItem.querySelector('.pizza-item--name').innerHTML = item.name;
  
//selecionando a descrição do item. Usando também o template String
...
c('.pizzaInfo--addButton').addEventListener('click', ()=> {
    //Para adicionar informações no carrinho precisa saber
    //Qual a pizza
    ...
    // adicionar identificador a Pizza pelo código e tamanho.
...
///Abrir menu lateral após adicionar pizza ao carrinho na versão mobile
c('.menu-openner').addEventListener('click', () => {
    if(cart.length > 0) {
      c('aside').style.left = '0';
    }
});
/////////////// Função de atualizar o carrinho de compras
function updateCart() {
    c('.menu-openner span').innerHTML = cart.length;

  if(cart.length > 0) {
    c('aside').classList.add('show');
  //zerar HTML para não efetuar a repetição
    c('.cart').innerHTML = "";
